...
import usePerson from '../hooks/usePerson'; // Assuming usePerson.tsx is in a 'hooks' directory

const Person: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { profileData, loading, error } = usePerson(id);
  ...
  if (!profileData) {
    return <div className="text-gray-800">No profile data available.</div>;
  }
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-6xl m-6">
        <div className="p-8">
          ...
          <div className="mt-4 text-justify text-black">
            ...
            {activeTab === 'publications' && id && <Publication userId={Number(id)} />}
            {activeTab === 'projects' && id && <Project userId={Number(id)}/>}
            {activeTab === 'supervisions' && id && <SupervisionsContent userId={Number(id)}/>}
            {activeTab === 'jury' && id && <JuryContent userId={Number(id)}/>}
            {activeTab === 'invitations' && id && <Invitation userId={Number(id)}/>}
            {activeTab === 'teaching' && id && <Teaching userId={Number(id)} />}
          </div>
        </div>
      </div>
  );
